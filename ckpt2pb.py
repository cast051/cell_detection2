...
def ckpt2pb():
     ...
     with tf.Session() as sess:
          ...
          info = tf.constant("19-1_1.0_2020-12-09;", dtype=tf.string)
...
def ckpt2pb_without_postprocess():
     def get_info():
          return info
     def get_info_error():
          return info_error
     ...
     with tf.Session() as sess:
          ...
          #save model
          constant_graph = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def, ["output","info"])
          with tf.gfile.FastGFile(config.pb_path, mode='wb') as f:
               f.write(constant_graph.SerializeToString())
          saver.save(sess, config.newckpt_path)
          print("%d ops in the final graph." % len(constant_graph.node))
